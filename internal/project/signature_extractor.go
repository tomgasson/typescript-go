...
// signatureExtractionResult wraps the extracted declarations along with a flag
// that indicates whether the extraction covered all encountered exports. If we
// see an unsupported export form, we return ok=false so callers can fall back
// to full parsing.
type signatureExtractionResult struct {
	declarations string
	ok           bool
}

func extractSignatures(src []byte) signatureExtractionResult {
	res := signatureExtractionResult{ok: true}
	i := 0
	n := len(src)
	var outParts []string
	...
	res.declarations = strings.Join(outParts, "\n") + "\n"
	return res
}
