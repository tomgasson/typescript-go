...
func TestRefCountingCaches(t *testing.T) {
	...
	t.Run("parseCache", func(t *testing.T) {
		...
		t.Run("reuse unchanged file", func(t *testing.T) {
			...
			mainEntry, _ := session.parseCache.entries.Load(NewParseCacheKey(main.ParseOptions(), main.Hash, main.ScriptKind, main.UseSignatureText()))
			utilsEntry, _ := session.parseCache.entries.Load(NewParseCacheKey(utils.ParseOptions(), utils.Hash, utils.ScriptKind, utils.UseSignatureText()))
			...
			newMainEntry, _ := session.parseCache.entries.Load(NewParseCacheKey(newMain.ParseOptions(), newMain.Hash, newMain.ScriptKind, newMain.UseSignatureText()))
		...
		t.Run("release file on close", func(t *testing.T) {
			...
			assert.Equal(t, utilsEntry.refCount, 1)
			assert.Equal(t, mainEntry.refCount, 0)
			mainEntry, ok := session.parseCache.entries.Load(NewParseCacheKey(main.ParseOptions(), main.Hash, main.ScriptKind, main.UseSignatureText()))
